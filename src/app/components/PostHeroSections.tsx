...
export function PostHeroSections() {
  ...
  const packagesByCategory: Record<string, any[]> = {
    ...
    Diabetes: [
      {
        title: "Diabetes Screening Profile",
        oldPrice: 1200,
        newPrice: 850,
        params: 12,
        time: 8,
    ...
    Thyroid: [
      ...
      {
        title: "CBC With ESR 1 (CBC+PS+ESR)",
        oldPrice: 570,
        newPrice: 350,
        params: 90,
        time: 6,
      ...
      {
        title: "CBC With ESR 2 (CBC+PS+ESR)",
        oldPrice: 570,
        newPrice: 350,
        params: 90,
        time: 6,
      ...
      {
        title: "CBC With ESR 3 (CBC+PS+ESR)",
        oldPrice: 570,
        newPrice: 350,
        params: 90,
        time: 6,
      ...
      {
        title: "CBC With ESR 4 (CBC+PS+ESR)",
        oldPrice: 570,
        newPrice: 350,
        params: 90,
        time: 6,
  ...
  const onSelect = useCallback(
    (emblaApi: any) => {
      ...
      setCanScrollNext(
        emblaApi.canScrollNext() || targetIndex.current < totalItems - 1,
      );
    },
    [totalItems],
  );
  ...
  return (
    <div className="w-full bg-white font-sans flex flex-col items-center">
      ...
      <div className="w-full bg-transparent pt-10 lg:pt-0 pb-4 lg:pb-0 lg:border-none">
        <div className="max-w-[1280px] mx-auto w-full px-6 mb-4 lg:mb-20 relative group">
          <div className="text-center mb-6 lg:mb-8">
            <h2 className="text-[24px] lg:text-[34px] font-bold text-black tracking-tight mb-2 lg:mb-3">
              Most-Booked Health Packages
            </h2>
            <p className="text-[14px] lg:text-[22px] text-gray-800 font-medium">
          ...
          <div className="relative overflow-visible">
            ...
            >
              <svg
                ...
                className="absolute inset-0 w-full h-full drop-shadow-sm group-hover:drop-shadow-md transition-all"
              >
                <circle
                  cx="19"
                  cy="19"
            ...
            <div className="overflow-hidden w-full py-6 -my-2" ref={emblaRef}>
              <div className="flex items-stretch -ml-3 lg:-ml-8">
                {/* Combine packages with a View More card */}
                {[...activePackages, { type: "view-more" }].map(
                  (pkg, index) => {
                    ...
                    return (
                      <div
                        key={index}
                        className={`flex-[0_0_auto] w-[calc(100vw-72px)] sm:w-[calc(100vw-88px)] md:w-[400px] pl-3 sm:pl-4 md:pl-8 ${isLast ? "pr-4 md:pr-10" : ""}`}
                      >
                        <div className="bg-white border border-gray-100 rounded-2xl shadow-sm relative h-full flex flex-col hover:shadow-[0_12px_40px_rgba(0,0,0,0.06)] transition-all duration-300">
                          {pkg.discount && (
                            <img
                              src={discountPill}
                              alt="Discount"
                              className="absolute top-[-14px] right-[-6px] sm:top-[-18px] sm:right-[-10px] md:top-[-25px] md:right-[-20px] w-10 sm:w-12 md:w-16 h-auto z-20 drop-shadow-md"
                            />
                          )}

                          <div className="bg-gradient-to-r from-[#0C1E40] to-[#16458B] px-4 py-4 sm:p-6 text-white flex justify-between items-start min-h-[90px] sm:h-[110px] rounded-t-2xl">
                            <h3 className="font-bold text-[13px] sm:text-[14px] md:text-[16px] max-w-[62%] leading-[1.2] tracking-tight">
                              {pkg.title}
                            </h3>

                            <div className="text-right whitespace-nowrap">
                              <div className="text-[10px] sm:text-[12px] text-blue-200/70 line-through mb-0.5">
                                ₹ {pkg.oldPrice}
                              </div>
                              <div className="font-bold text-[18px] sm:text-[20px] md:text-[22px] leading-none">
                                ₹ {pkg.newPrice}
                              </div>
                            </div>
                          </div>
                          <div className="p-6 flex-1 flex flex-col justify-between bg-[#F8FAFC]">
                            <div className="flex items-center justify-between gap-4 mb-4 sm:mb-5">
                              ...
                              <div className="flex items-start gap-2 flex-1">
                                <img
                                  src={reportIcon}
                                  alt="parameters"
                                  className="w-[14px] h-[14px] sm:w-[16px] sm:h-[16px] mt-[2px] opacity-70"
                                />
                                <div className="text-[11px] sm:text-[12px] text-gray-700 font-medium leading-tight">
                                  <div className="whitespace-nowrap">
                                    {pkg.params} parameters
                                  </div>
                                  <div className="text-gray-500 text-[10px] sm:text-[11px]">
                                    included
                                  </div>
                                </div>
                              </div>
                            ...
                            <div className="flex items-center gap-2 sm:gap-3 w-full mt-auto">
                              <button className="flex-1 py-2.5 sm:py-3 border border-[#0076BC] text-[#0076BC] rounded-xl font-semibold text-[12px] sm:text-[14px] hover:bg-blue-50 active:scale-[0.98] transition-all bg-white">
                                View Details
                              </button>
                              <button className="flex-1 py-2.5 sm:py-3 bg-[#0076BC] text-white rounded-xl font-semibold text-[12px] sm:text-[14px] hover:bg-[#0A366B] active:scale-[0.98] transition-all">
                                Add to cart
                              </button>
                            </div>
                          </div>
      ...
      <div className="w-full py-8 lg:py-16 flex flex-col items-center bg-white relative overflow-hidden">
        ...
        <div className="relative z-10 w-full flex flex-col items-center">
          <h2 className="text-[24px] lg:text-[60px] font-bold text-gray-900 mb-2 text-center leading-tight">
            What brings you here today?
          </h2>
          <p className="text-gray-700 mb-6 lg:mb-10 text-[14px] lg:text-[20px] text-center">
            ...
            <div className="bg-white rounded-xl p-2.5 lg:p-5 w-full lg:w-[280px] shadow-[0_8px_24px_rgba(0,0,0,0.08)] border border-transparent hover:border-blue-300 hover:shadow-[0_12px_28px_rgba(0,0,0,0.12)] transition-all duration-300 cursor-pointer flex flex-col h-[72px] lg:h-[90px] justify-center items-center lg:items-start text-center lg:text-left relative">
              <div className="flex flex-col lg:flex-row items-center lg:items-center gap-1.5 lg:gap-3 w-full">
                ...
                <div className="flex flex-col flex-1">
                  <span className="font-semibold text-[11px] lg:text-[16px] text-gray-900 leading-tight lg:leading-none">
                    I know my test
                  </span>
                </div>
              </div>
        ...
        <div className="bg-[#071930] rounded-[24px] md:rounded-[32px] min-h-0 md:min-h-[520px] text-white relative overflow-hidden flex flex-col">
          <div className="flex flex-col md:flex-row justify-between items-center mb-4 md:mb-16 relative z-10 w-full gap-5 md:gap-8 p-6 md:p-14">
            <div className="flex-1 max-w-sm text-center md:text-left">
              <h2 className="text-[24px] md:text-[38px] font-bold leading-tight mb-3 md:mb-4 text-[#8CBEF0]">
                Your Trusted
                <br className="hidden md:block" />
                Lab Partner
              </h2>
              <p className="text-gray-300 text-[14px] md:text-sm leading-relaxed">
                Since 1995, Agilus Diagnostics (formerly SRL) has built a legacy
                of expertise, experience and trust with patients, doctors, and
          ...
          <div className="grid grid-cols-2 md:grid-cols-4 gap-y-5 md:gap-y-8 gap-x-3 md:gap-x-4 w-full relative z-10 px-4 md:px-14 pb-8 md:pb-10">
            {[
              ...
              <div key={idx} className="text-center">
                <div className="text-2xl md:text-4xl font-bold text-[#8CBEF0] mb-1">
                  {stat.value}
                </div>
                <div className="text-[9px] md:text-xs text-gray-300 tracking-wide uppercase font-semibold">
                  {stat.label}
                </div>
              </div>
            ))}
